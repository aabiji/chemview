...
impl Renderer {
    pub async fn new(window: Arc<Window>) -> Self {
        ...
        let msaa_texture = Renderer::create_msaa_texture(
            &device,
        ...
        let depth_texture =
            Renderer::create_depth_texture(&device, window_size.width, window_size.height);
    ...
    pub fn resize(&mut self, size: PhysicalSize<u32>) {
        ...
        self.msaa_texture = Renderer::create_msaa_texture(
            &self.device,
            self.surface_format.add_srgb_suffix(),
            size.width,
            size.height,
        );
        self.depth_texture = Renderer::create_depth_texture(&self.device, size.width, size.height);
